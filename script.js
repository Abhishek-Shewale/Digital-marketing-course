...
const topics = [
  ...
  Treat content as an iterative product: test formats, measure outcomes, and iterate. Over time this creates a compounding library of assets that drive predictable value.`,
  ...
  
    {
      ...
      cover: 'https://images.unsplash.com/photo-1495020689067-958852a7765e?q=80&w=1400&auto=format&fit=crop',
  ...
  Tie awareness work to business outcomes when possible: does increased recall translate into more branded search or better conversion efficiency?`,
  ...
  
    {
      ...
      cover: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?q=80&w=1400&auto=format&fit=crop&ixlib=rb-4.0.3',
  ...
  function downloadSyllabus() {
    if (!currentCourse) return;
    const header = `# ${currentCourse.title}\n\n${currentCourse.desc}\n\nLevel: ${currentCourse.level} • ${currentCourse.duration}\n\nLessons:\n\n`;
    const lessonsText = (currentCourse.lessons || []).map((lesson, idx) => {
      const t = `${idx + 1}. ${lesson.title}\n`;
      const c = lesson.content ? `\n${lesson.content.replace(/\r/g,'')}\n` : '\n';
      return t + c + '\n';
    }).join('\n');
  
    const text = header + lessonsText + '\n---\nGenerated by Digital Marketing Micro-site\n';
    const blob = new Blob([text], { type: 'text/markdown;charset=utf-8' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `${(currentCourse.title || 'syllabus').replace(/[^a-z0-9]+/gi,'-').toLowerCase()}-syllabus.md`;
